  ...
  bodyAttr(config){
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        const postData  = { image: config.image , type: config.baidu_type  }

        const { data } = await axios.post(`https://aip.baidubce.com/rest/2.0/image-classify/v1/body_attr?access_token=${token}`, qs.stringify(postData), {
          headers: { 'Content-Type': '	application/x-www-form-urlencoded' }
        }).catch(err => {
          throw new Error(`[百度body attr]${err}`)
        })

        if (data.error_code > 0) {
          const msg = BaiduCode[data.error_code] || data.error_msg
          throw new Error(`[百度body attr]${msg}`)
        }
